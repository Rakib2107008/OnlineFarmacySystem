...
(function() {
  ...
  setTimeout(() => {
    ...
    } catch(e) { console.warn('Cart hydrate fallback failed on vitaminSupplyments page:', e); }
  }, 0);
  ...
  document.addEventListener('click', function(e) {
    ...
    const productId   = button.getAttribute('data-product-id') || card?.getAttribute('data-product-id') || productName.toLowerCase().replace(/\s+/g,'-');
    const priceAttr   = button.getAttribute('data-product-price');
    const priceText   = card?.querySelector('.current-price')?.textContent || '0';
    const unitPrice   = toNumber(priceAttr ?? priceText, 0);
