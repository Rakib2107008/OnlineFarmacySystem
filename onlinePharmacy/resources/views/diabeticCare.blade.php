...
<style>
  ...
  .page-item.disabled .page-link:hover { transform: none; background: #fafafa; }
  ...
  @media (max-width: 576px) {
    ...
    .product-card { margin-bottom: 20px; padding: 15px; }
    .product-img { height: 160px; }
    .product-name { font-size: 14px; min-height: 40px; margin-bottom: 10px; }
    .current-price { font-size: 18px; }
    .old-price { font-size: 14px; }
    .btn-add-cart { padding: 10px; font-size: 14px; }
    .pagination { gap: 4px; }
    .page-link { min-width: 35px; height: 35px; padding: 6px 10px; font-size: 14px; }
...
<script>
(function() {
  // ---- Ensure cart UI is hydrated immediately on this page (matches other pages) ----
  ...
  setTimeout(() => {
    ...
    try {
      const stateRaw = localStorage.getItem('floatingCartState') || '{"items": []}';
      ...
      console.warn('Cart hydrate fallback failed on diabeticCare page:', e);
    }
  }, 0);

  // ---- Page script (mirrors personalCare), with correct template strings ----
  ...
  document.addEventListener('click', (event) => {
    ...
    const productName = (button.dataset.productName || productCard?.querySelector('.product-name')?.textContent || '').trim() || 'Product';
    const productId   = button.dataset.productId || productCard?.getAttribute('data-product-id') || productName.toLowerCase().replace(/\s+/g, '-');
    const priceSource = button.dataset.productPrice || productCard?.querySelector('.current-price')?.textContent || '0';
    ...
    // Try immediately; if not ready, retry on next tick once.
    const performAdd = () => {
  ...
  if (carouselElement) {
    const carousel = new bootstrap.Carousel(carouselElement, { interval: 3000, wrap: true, keyboard: true, pause: 'hover' });
  }
