...
<section class="products-section">
  <div class="container" id="productContainer">
    <h2 class="section-title">Sexual Wellbeing</h2>
...
<script>
(function() {
  // --- Ensure cart badge and total persist correctly ---
  ...
  setTimeout(() => {
    ...
    try {
      ...
      console.warn('Cart rehydrate failed on sexualWellbeing:', e);
    }
  }, 0);
  ...
  (products || []).forEach(product => {
    ...
    const currPrice = toNumber(product.current_price ?? product.price ?? 0);
    ...
    const btn = colDiv.querySelector('.btn-add-cart');
    if (btn) {
      btn.dataset.productId = product.id ?? '';
      btn.dataset.productName = product.name ?? '';
      btn.dataset.productPrice = currPrice;
    }
  });
  ...
  document.addEventListener('click', (event) => {
    ...
    const name = button.dataset.productName || card?.querySelector('.product-name')?.textContent || 'Product';
    const id = button.dataset.productId || name.toLowerCase().replace(/\s+/g,'-');
